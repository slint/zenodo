...
from zenodo.modules.utils.tasks import (
    comm_sets_match,
    get_synced_sets,
    requires_sync,
...
def test_synced_communities(db, oaisets):
    ...
    assert get_synced_sets(make_rec(["c1", "c2"], ["user-c1", "user-c2"])) == [
        "user-c1",
        "user-c2",
    ]
    assert get_synced_sets(make_rec(["c1", "c2"], [])) == ["user-c1", "user-c2"]
    assert get_synced_sets(make_rec(["c1", "c2"], ["extra", "user-c1", "user-c2"])) == [
        "extra",
        "user-c1",
        "user-c2",
    ]
    ...
    assert get_synced_sets(
        make_rec(["c1", "c2"], ["user-c1", "user-c2", "user-extra"])
...
def test_sets_match(db, oaisets):
    ...
    assert comm_sets_match(make_rec(["c1", "c2"], ["extra", "user-c1", "user-c2"]))
    # Should also ignore the custom "user-extra" spec which is NOT
    # community-based but has a prefix as such
    assert comm_sets_match(make_rec(["c1", "c2"], ["user-c1", "user-c2", "user-extra"]))
    ...
    r = {"communities": ["c1"], "_oai": {"id": "some_id_1234", "updated": "timestamp"}}
