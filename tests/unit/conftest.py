...
from invenio_deposit.permissions import action_admin_access as deposit_admin_access
...
def use_flask_limiter(app):
    ...
    flask_current_app.config.update(
        dict(
...
def default_config(tmp_db_path):
    ...
    ZENODO_OPENAIRE_COMMUNITIES = {
        ...
        "bar": {
            ...
            "communities": ["c3", "c1"],
            "types": {
                "software": [{"id": "bar:t3", "name": "Bar sft type three"}],
                "other": [{"id": "bar:t6", "name": "Bar other type six"}],
            },
        },
    }
    ...
    return dict(
        ...
        SQLALCHEMY_DATABASE_URI=os.environ.get("SQLALCHEMY_DATABASE_URI", tmp_db_path),
        TESTING=True,
        ...
        ZENODO_CUSTOM_METADATA_VOCABULARIES={
            "dwc": {
                ...
                "attributes": {
                    "family": {"type": "keyword", "multiple": False},
                    "genus": {"type": "keyword", "multiple": True},
                    "behavior": {"type": "text", "multiple": False},
                },
            }
        },
    )
...
def locations(db):
    ...
    arch_loc = Location(name="archive", uri=arch_tmppath, default=False)
    db.session.add(loc)
...
def users(app, db):
    ...
    user_admin = create_test_user(email="admin@zenodo.org", password="admin")

    with db.session.begin_nested():
        # set admin permissions
        db.session.add(ActionUsers(action=action_admin_access.value, user=user_admin))
        db.session.add(ActionUsers(action=deposit_admin_access.value, user=user_admin))
    db.session.commit()
...
def oaisets(db, communities):
    ...
    """
    oaisets_data = [
        {"spec": "extra", "search_pattern": "title:extra"},
        {
...
def test_object(db, bucket):
    ...
    obj = ObjectVersion.create(
        bucket, "test.txt", stream=BytesIO(data_bytes), size=len(data_bytes)
    )
    db.session.commit()
...
def full_record():
    ...
    record = dict(
        ...
        related_identifiers=[
            {"identifier": "10.1234/foo.bar", "scheme": "doi", "relation": "cites"},
            {"identifier": "1234.4325", "scheme": "arxiv", "relation": "isIdenticalTo"},
            {"identifier": "1234.4321", "scheme": "arxiv", "relation": "cites"},
            {"identifier": "1234.4328", "scheme": "arxiv", "relation": "references"},
            {
                "identifier": "10.1234/zenodo.4321",
                "scheme": "doi",
                "relation": "isPartOf",
            },
            {
        ...
        contributors=[
            ...
            {"affiliation": "CERN", "name": "Kowalski, Manager", "type": "DataManager"},
        ],
        references=[
            {
        ...
        imprint={"place": "Staszkowka", "publisher": "Jol", "isbn": "978-0201633610"},
...
def funder_record(db):
    ...
    funder = Record.create(
        dict(doi="10.13039/501100000780", name="European Commission", acronyms=["EC"])
    )
    PersistentIdentifier.create(
        pid_type="frdoi",
    ...
    grants = [
        Record.create(
            dict(
                ...
                identifiers=dict(eurepo="info:eu-repo/grantAgreement/EC/FP7/282896"),
                code="282896",
        ...
        Record.create(
            dict(
                ...
                funder={"$ref": "https://dx.doi.org/10.13039/501100000780"},
                identifiers=dict(eurepo="info:eu-repo/grantAgreement/EC/FP6/027819"),
                code="027819",
...
def resolver():
    ...
    return Resolver(pid_type="recid", object_type="rec", getter=ZenodoRecord.get_record)
    ...
    return [
        {
            ...
            "extra_data": {
                "repos": {
                    "8000": {"full_name": "foo/bar"},
                    "8002": {"full_name": "bacon/eggs"},
                    "8003": {"full_name": "other/repo"},
                }
            },
        }
    ]
...
def g_remoteaccounts(app, db, g_remoteaccounts_data):
    ...
    for rad in g_remoteaccounts_data:
        ra = RemoteAccount.create(rad["user_id"], "changeme", rad["extra_data"])
        db.session.add(ra)
        db.session.commit()
        rad["id"] = ra.id
...
def g_tester_id(app, db):
    ...
    datastore = app.extensions["security"].datastore
    tester = datastore.create_user(email="info@inveniosoftware.org", password="tester")
    db.session.commit()
    return tester.id
...
def sample_identifiers():
    ...
    return {
        ...
        "arxiv": ("hep-th/1601.07616", "https://arxiv.org/abs/arXiv:1601.07616"),
        "bioproject": (
            "PRJNA224116",
        ...
        "doi": ("10.1002/example", "https://doi.org/10.1002/example"),
        "ean13": ("4006381333931", ""),
        "ean8": ("73513537", ""),
        "ensembl": (
            "ENSMUST00000017290",
        ...
        "hal": ("mem_13102590", "https://hal.archives-ouvertes.fr/mem_13102590"),
        "handle": ("10013/epic.10033", "https://hdl.handle.net/10013/epic.10033"),
        "isbn": ("0-9752298-0-X", ""),
        ...
        "pmcid": ("PMC2631623", "https://www.ncbi.nlm.nih.gov/pmc/PMC2631623"),
        "pmid": ("pmid:12082125", "https://www.ncbi.nlm.nih.gov/pubmed/12082125"),
        "purl": ("http://purl.oclc.org/foo/bar", "http://purl.oclc.org/foo/bar"),
        "refseq": (
            "NZ_JXSL01000036.1",
            "https://www.ncbi.nlm.nih.gov/entrez/viewer.fcgi" "?val=NZ_JXSL01000036.1",
        ),
        "sra": ("SRR6437777", "https://www.ebi.ac.uk/ena/data/view/SRR6437777"),
        "uniprot": ("Q9GYV0", "https://purl.uniprot.org/uniprot/Q9GYV0"),
        "url": (
