...
def test_basic_api(app, db, communities, deposit, deposit_file):
    ...
    # to all versions
    assert record_v1["communities"] == record_v2["communities"] == ["c1", "c2"]
...
def test_autoadd(
    ...
    # Accepting individual record to a community should propagate the changes
    # to all versions
    assert record_v1.get("communities", []) == ["c1", "c2", "c3", "grants_comm"]
    assert record_v2.get("communities", []) == ["c1", "c2", "c3", "grants_comm"]
    assert deposit_v1.get("communities", []) == [
        "c1",
        "c2",
        "c3",
    ...
    assert deposit_v1.get("communities", []) == ["ecfunded", "grants_comm", "zenodo"]
    assert deposit_v2.get("communities", []) == ["ecfunded", "grants_comm", "zenodo"]
