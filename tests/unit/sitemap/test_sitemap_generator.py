...
def test_sitemap_generators(app, record_with_bucket, communities):
    ...
    with app.test_request_context():
        ...
        assert all(
            "lastmod" in url and sitemap_dt_re.match(url["lastmod"]) for url in urls
        )
        for url in urls:
            del url["lastmod"]
        expected = [
