...
def test_hook_sync(mocker, app, cli_run, g_tester_id):
    ...
    mock_obj = mocker.patch.object(GitHubAPI, "sync")
    ret = cli_run("sync info@inveniosoftware.org --hooks True" " --async-hooks=True -E")
    assert ret.exit_code == 0
    assert ret.output == ""
...
def test_hook_create(mocker, app, cli_run, g_users, g_repositories):
    ...
    mock_obj = mocker.patch.object(GitHubAPI, "create_hook")
    ret = cli_run("createhook u1@foo.bar {0} --yes-i-know -E".format(repo["github_id"]))
    assert ret.output == ""
    assert ret.exit_code == 0
    mock_obj.assert_called_once_with(repo["github_id"], repo["name"])
...
def test_hook_remove(mocker, app, cli_run, g_users, g_repositories):
    ...
    assert ret.exit_code == 0
    assert (
        ret.output == "Warning: Specified user is not the owner of this repository.\n"
    )
    mock_obj.assert_called_once_with(repo0["github_id"], repo0["name"])
    ...
    assert ret.exit_code == 0
    assert ret.output == "Repository doesn't have an owner, please specify a user.\n"
    assert not mock_obj.called
...
def test_repo_assign_many(mocker, r2, r1, u2, app, cli_run, g_users, g_repositories):
    """Test 'assign' CLI."""
