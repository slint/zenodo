...
def test_archiving(app, db, deposit, deposit_file, locations, archive_fs):
    ...
    sip1_bagmeta = json.loads(
        next(m.content for m in sip1.metadata if m.type.name == "bagit")
    )["files"]
    sip2_bagmeta = json.loads(
        next(m.content for m in sip2.metadata if m.type.name == "bagit")
    )["files"]
    sip3_bagmeta = json.loads(
        next(m.content for m in sip3.metadata if m.type.name == "bagit")
    )["files"]

    # Check if Bagit metadata contains the correct file-fetching information
    assert set([f["filepath"] for f in sip1_bagmeta]) == set(
        [
            s1_file1_fp,
            s1_file2_fp,
            "data/filenames.txt",
            "data/metadata/record-json.json",
            "bag-info.txt",
            "manifest-md5.txt",
            "bagit.txt",
            "tagmanifest-md5.txt",
    ...
    assert set([f["filepath"] for f in sip2_bagmeta]) == set(
        [
            s1_file1_fp,
            s1_file2_fp,
            "data/filenames.txt",
    ...
    assert set([f["filepath"] for f in sip3_bagmeta]) == set(
        [
            s3_file2_fp,
            s3_file3_fp,
            "data/filenames.txt",
    ...
    assert set(fs1.listdir()) == set(
        ["tagmanifest-md5.txt", "bagit.txt", "manifest-md5.txt", "bag-info.txt", "data"]
    )
    assert set(fs1.listdir("data")) == set(["metadata", "files", "filenames.txt"])
    ...
    assert set(cnt) == set(
        [
            "{base}/2/r/{s1ts}/{fn} 4 {fn}".format(
                fn=s1_file1_fp, base=base_uri, s1ts=sip1_ts
            ),
            "{base}/2/r/{s1ts}/{fn} 8 {fn}".format(
                fn=s1_file2_fp, base=base_uri, s1ts=sip1_ts
            ),
        ]
    )
    ...
    assert set(fs3.listdir("data")) == set(["metadata", "files", "filenames.txt"])
    ...
    assert set(cnt) == set(
        [
            "{base}/2/r/{s1ts}/{fn} 8 {fn}".format(
                fn=s3_file2_fp, base=base_uri, s1ts=sip1_ts
            )
        ]
    )
