...
def test_minimal(db, minimal_record_model, recid_pid):
    ...
    assert obj == {
        ...
        "subjects": [],
        "contributors": [],
        "resourceType": {"resourceType": None, "resourceTypeGeneral": "Software"},
...
def test_non_local_doi(db, minimal_record_model, recid_pid):
    ...
    assert obj["identifier"] == {
        "identifier": "http://localhost/record/123",
        "identifierType": "URL",
    }
...
def test_full(db, record_with_bucket, recid_pid):
    ...
    expected = {
        "alternateIdentifiers": [
            ...
            {"alternateIdentifier": "0317-8471", "alternateIdentifierType": "issn"},
            {
        ...
        "creators": [
            ...
            {"affiliation": "CERN", "creatorName": "Smith, John", "nameIdentifier": {}},
            {
                "affiliation": "CERN",
        ...
        "descriptions": [
            ...
            {"description": "notes", "descriptionType": "Other"},
            {
                "description": (
            ...
            {"description": "microscopic supersampling", "descriptionType": "Methods"},
        ],
        "identifier": {"identifier": "10.5072/foo", "identifierType": "DOI"},
        "language": "en",
        "geoLocations": [
            {"geoLocationPlace": "my place", "geoLocationPoint": "2.35 1.534"},
        ...
        "resourceType": {"resourceType": "Book", "resourceTypeGeneral": "Text"},
        "rightsList": [
            {
    ...
    expected["creators"] = [
        {
            ...
            "nameIdentifiers": [
                ...
                {"nameIdentifierScheme": "GND", "nameIdentifier": "170118215"},
            ],
        },
        {
            "affiliations": ["CERN"],
    ...
    expected["dates"] = [
        {"date": "2014-02-27", "dateType": "Issued"},
        {"date": "2019-01-01/", "dateType": "Valid", "dateInformation": "Bongo"},
        {"date": "/2019-01-01", "dateType": "Collected"},
        {"date": "2019-01-01", "dateType": "Withdrawn"},
        {"date": "2019-01-01/2019-02-01", "dateType": "Collected"},
    ]
    expected["geoLocations"] = [
        {
            ...
            "geoLocationPoint": {"pointLatitude": 2.35, "pointLongitude": 1.534},
        },
        {"geoLocationPlace": "New York"},
    ]
    assert obj == expected
...
@pytest.mark.parametrize("serializer", [datacite_v31, datacite_v41])
    ...
    assert obj["creators"] == [
        {
            "affiliation": "AA",
            "creatorName": "A",
            "nameIdentifier": {"nameIdentifier": "1234", "nameIdentifierScheme": "GND"},
        },
        {
            "affiliation": "BA",
            "creatorName": "B",
    ...
    assert obj["creators"] == [
        ...
        },
        {
            ...
            "nameIdentifiers": [
                ...
                {"nameIdentifier": "4321", "nameIdentifierScheme": "GND"},
            ],
        },
    ]
...
def test_embargo_date(db, minimal_record_model, recid_pid, serializer):
    ...
    minimal_record_model.update({"embargo_date": dt, "access_right": "embargoed"})
    obj = serializer.transform_record(recid_pid, minimal_record_model)
    assert obj["dates"] == [
...
def test_contributors(db, minimal_record_model, recid_pid):
    ...
    minimal_record_model.update(
        {
            "contributors": [
                {"name": "A", "affiliation": "AA", "gnd": "1234", "type": "Researcher"}
            ],
            "thesis_supervisors": [
                {"name": "B", "affiliation": "BA", "type": "Supervisor"}
            ],
            "grants": [
                {
...
def test_resource_type(db, minimal_record_model, recid_pid, serializer):
    ...
    minimal_record_model["resource_type"] = {
        "type": "software",
...
def test_alt_ids(db, minimal_record_model, recid_pid, serializer):
    """Test language."""
    minimal_record_model.update(
        {"alternate_identifiers": [{"identifier": "10.1234/foo.bar", "scheme": "doi"}]}
    )
    obj = serializer.transform_record(recid_pid, minimal_record_model)
    assert obj["alternateIdentifiers"] == [
        {"alternateIdentifier": "10.1234/foo.bar", "alternateIdentifierType": "doi"},
        {
            "alternateIdentifier": "http://localhost/record/123",
            "alternateIdentifierType": "url",
        },
...
def test_related_identifiers(db, minimal_record_model, recid_pid, serializer):
    ...
    for t, dc_t in tests:
        minimal_record_model.update(
            {
                "related_identifiers": [
                    {"identifier": "1234", "scheme": t, "relation": "isCitedBy"},
                    {
                        "identifier": "1234",
                        "scheme": "invalid",
                        "relation": "isCitedBy",
...
def test_communities_rel_ids(db, minimal_record_model, recid_pid, serializer):
    ...
    for communities in (["zenodo"], ["c1", "c2", "c3"]):
        ...
        for comm in communities:
            assert {
                "relatedIdentifier": "http://localhost/communities/{}".format(comm),
                "relatedIdentifierType": "URL",
                "relationType": "IsPartOf",
...
def test_descriptions(db, minimal_record_model, recid_pid, serializer):
    ...
    assert obj["descriptions"] == [
        ...
        {"description": "again", "descriptionType": "Other"},
        {"description": json.dumps({"references": ["A"]}), "descriptionType": "Other"},
    ]
    ...
    obj = serializer.transform_record(recid_pid, minimal_record_model)
    assert all(
        len(d["description"]) == 20000 and "B" not in d["description"]
    ...
    assert obj["fundingReferences"] == [
        {
            ...
            "awardNumber": {"awardNumber": "1234", "awardURI": "eurepo 1"},
        ...
        },
        {
            ...
            "awardNumber": {"awardNumber": "4321", "awardURI": "eurepo 2"},
