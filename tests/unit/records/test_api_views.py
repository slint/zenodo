...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("2.45,1.63,NaN,-1.53", 400, 'Invalid number: "NaN" is not a permitted value.'),
        ("2.45,1.63,Infinity,-1.53", 400, "Longitude must be between -180 and 180."),
        (
            "-1.43,1.63,2.45,-1.53",
            400,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "[dwc:family]",
            400,
            "The parameter should have the " "format: custom=[field_name]:field_value.",
        ),
        (
            ":Felidae",
            400,
...
@pytest.mark.parametrize(
    ("query", "result"), [("[dwc:family]:Felidae", 1), ("[dwc:family]:foobar", 0)]
)
def test_custom_search(
    es, api, json_headers, record_with_bucket, custom_metadata, query, result
):
    ...
    with api.test_request_context():
        with api.test_client() as client:
            res = client.get(
                url_for("invenio_records_rest.recid_list", custom=query),
                headers=json_headers,
            )
