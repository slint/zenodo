...
def minimal_oai_record(minimal_record):
    """Minimal OAI record."""
    minimal_record["_oai"] = {"id": "oai:zenodo.org:{}".format(minimal_record["recid"])}
    minimal_record["resource_type"] = {"type": "publication", "subtype": "article"}
...
def full_oai_record(full_record):
    """Minimal OAI record."""
    full_record["_oai"] = {"id": "oai:zenodo.org:{}".format(full_record["recid"])}
    full_record["resource_type"] = {"type": "publication", "subtype": "article"}
    return full_record


def test_minimal(app, db, minimal_oai_record, recid_pid):
    """Test minimal record."""
    obj = openaire_json_v1.transform_record(recid_pid, Record(minimal_oai_record))
    assert obj == {
...
def test_full(app, db, full_oai_record, recid_pid):
    """Test minimal record."""
    obj = openaire_json_v1.transform_record(recid_pid, Record(full_oai_record))
    assert obj == {
        ...
        "pids": [
            {"type": "oai", "value": "oai:zenodo.org:12345"},
...
def test_grants(app, db, minimal_oai_record, recid_pid):
    ...
    minimal_oai_record["grants"][0]["title"] = "Üniçoδé Grànt Title"
    obj = openaire_json_v1.transform_record(recid_pid, Record(minimal_oai_record))
    assert obj["linksToProjects"] == [
        "info:eu-repo/grantAgreement/EC/FP7/244909/" "/Üniçoδé Grànt Title/WorkAble"
    ]
