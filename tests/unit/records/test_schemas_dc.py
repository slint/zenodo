...
def test_minimal(app, db, minimal_record_model, recid_pid):
    ...
    assert obj == {
        ...
        "types": ["info:eu-repo/semantics/other", "software"],
        "relations": [],
    }
...
def test_identifiers(app, db, minimal_record_model, recid_pid):
    ...
    obj = dc_v1.transform_record(recid_pid, minimal_record_model)
    assert obj["identifiers"] == ["https://zenodo.org/record/123", "10.1234/foo"]
...
def test_relations(app, db, minimal_record_model, recid_pid):
    ...
    minimal_record_model.update(
        {
            "grants": [
                {"identifiers": {"eurepo": "info:eu-repo/grantAgreement/EC/FP7/244909"}}
            ],
            "alternate_identifiers": [
                {"identifier": "10.1234/foo.bar", "scheme": "doi"}
            ],
            "related_identifiers": [
                {"identifier": "1234", "scheme": "pmid", "relation": "isCited"}
            ],
        }
    )
    obj = dc_v1.transform_record(recid_pid, minimal_record_model)
...
def test_publishers(app, db, minimal_record_model, recid_pid):
    """Test publishers."""
    minimal_record_model.update({"part_of": {"publisher": "Zenodo"}})
    obj = dc_v1.transform_record(recid_pid, minimal_record_model)
    assert obj["publishers"] == ["Zenodo"]
    minimal_record_model.update(
        {"imprint": {"publisher": "Invenio"}, "part_of": {"publisher": "Zenodo"}}
    )
    obj = dc_v1.transform_record(recid_pid, minimal_record_model)
...
def test_contributors(app, db, minimal_record_model, recid_pid):
    """"Test contributors."""
    minimal_record_model.update({"contributors": [{"name": "Smith, John"}]})
    obj = dc_v1.transform_record(recid_pid, minimal_record_model)
...
def test_types(app, db, minimal_record_model, recid_pid):
    ...
    minimal_record_model.update(
        {"resource_type": {"type": "software", "openaire_subtype": "foo:t1"}}
    )
    obj = dc_v1.transform_record(recid_pid, minimal_record_model)
    assert obj["types"] == ["info:eu-repo/semantics/other", "software"]
...
def test_community_relations(db, minimal_record_model, recid_pid):
    ...
    for communities in (["zenodo"], ["c1", "c2", "c3"]):
        ...
        for comm in communities:
            assert (
                "url:http://localhost/communities/{}".format(comm) in obj["relations"]
            )
...
def test_sources(app, db, minimal_record_model, recid_pid):
    ...
    minimal_record_model.update({"journal": {"title": "CAP", "issue": "1"}})
    obj = dc_v1.transform_record(recid_pid, minimal_record_model)
