...
def test_upload_type(minimal_record_model, depid_pid, legacyjson_v1):
    ...
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)["metadata"]
    assert obj["upload_type"] == "software"
    assert "publication_type" not in obj
    assert "image_type" not in obj

    minimal_record_model["resource_type"] = dict(type="publication", subtype="preprint")
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)["metadata"]
    assert obj["upload_type"] == "publication"
    assert obj["publication_type"] == "preprint"
    assert "image_type" not in obj

    minimal_record_model["resource_type"] = dict(type="image", subtype="photo")
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)["metadata"]
...
def test_creators(minimal_record_model, depid_pid, legacyjson_v1):
    ...
    minimal_record_model["thesis"] = dict(supervisors=minimal_record_model["creators"])
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)["metadata"]
    assert obj["creators"] == [
        {"name": "Doe, John"},
        {"name": "Smith, John", "affiliation": "CERN", "orcid": "1234", "gnd": "4321"},
    ]
...
def test_direct_mappings(minimal_record_model, depid_pid, legacyjson_v1):
    ...
    fields = ["title", "description", "notes", "access_right", "access_conditions"]
...
def test_prereserve(minimal_record_model, depid_pid, legacyjson_v1):
    ...
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)["metadata"]
    assert obj["prereserve_doi"] == {"recid": 123, "doi": "10.5072/zenodo.123"}
...
def test_related_identifiers(minimal_record_model, depid_pid, legacyjson_v1):
    ...
    minimal_record_model.update(
        dict(
            ...
            alternate_identifiers=[dict(identifier="10.1234/f", scheme="doi")],
        )
    )
    ...
    assert obj["related_identifiers"] == [
        dict(identifier="10.1234/f", scheme="doi", relation="cites"),
        dict(identifier="10.1234/f", scheme="doi", relation="isAlternateIdentifier"),
    ]
...
def test_imprint(minimal_record_model, depid_pid, legacyjson_v1):
    ...
    minimal_record_model.update(
        dict(
            imprint=dict(
                place="Some place", publisher="Some publisher", isbn="978-3-16-148410-0"
            )
        )
    )
...
def test_partof(minimal_record_model, depid_pid, legacyjson_v1):
    ...
    minimal_record_model.update(
        dict(
            ...
            part_of=dict(pages="Some pages", title="Some title"),
        )
    )
...
def test_state(minimal_record_model, depid_pid, legacyjson_v1):
    """Test state."""
    minimal_record_model.update(dict(_deposit=dict(status="draft")))
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)
    ...
    minimal_record_model.update(dict(_deposit=dict(status="published")))
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)
    ...
    minimal_record_model.update(
        dict(_deposit=dict(pid={u"revision_id": 0, u"type": u"recid", u"value": u"1"}))
    )
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)
    assert obj["state"] == "inprogress"
...
def test_submitted(minimal_record_model, depid_pid, legacyjson_v1):
    ...
    minimal_record_model.update(
        dict(_deposit=dict(status="published", pid=dict(type="recid", value="1")))
    )
    obj = legacyjson_v1.transform_record(depid_pid, minimal_record_model)
