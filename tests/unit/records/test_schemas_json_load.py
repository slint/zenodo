...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "10.5281/allow",
            dict(allowed_dois=["10.5281/allow"], managed_prefixes=["10.5281"]),
        ),
        (
            "10.5281/required",
            dict(
...
@pytest.mark.parametrize("val", ["2016-02-32", " invalid"])
...
def test_title(val, expected):
    """Test title."""
    data, errors = MetadataSchemaV1(partial=["title"]).load(dict(title=val))
...
def test_dates():
    ...
    data, errors = schema.load(
        {"dates": [{"type": "Valid", "start": "2019-02-01", "end": "2019-01-01"}]}
    )
    assert 'must be before "end"' in errors["dates"][0]

    # Single date value (i.e. start == end)
    data, errors = schema.load(
        {"dates": [{"type": "Valid", "start": "2019-01-01", "end": "2019-01-01"}]}
    )
    assert "dates" not in errors
    data, errors = schema.load({"dates": [{"type": "Valid", "start": "2019-01-01"}]})
    assert "dates" not in errors
    data, errors = schema.load({"dates": [{"type": "Valid", "end": "2019-01-01"}]})
    assert "dates" not in errors
    data, errors = schema.load(
        {
            "dates": [
...
def test_geographical_metadata(val, expected):
    ...
    data, errors = MetadataSchemaV1(partial=["locations"]).load(dict(locations=val))
    if expected is not None:
