...
def test_serializer(bibtex_records):
    ...
    results = {"hits": {"hits": [{"_source": test_record}]}}
    assert bibtex == serializer.serialize_search(
        search_result=results, pid_fetcher=None
    )
...
def test_get_entry_type(bibtex_records):
    ...
    assert test_record["resource_type"]["type"] == record_good._get_entry_type()
...
def test_get_entry_subtype(bibtex_records):
    """Test."""
    (record_good, record_bad, record_empty, test_record) = bibtex_records
    assert test_record["resource_type"]["subtype"] == record_good._get_entry_subtype()
