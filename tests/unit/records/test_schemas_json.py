...
def test_json_v1(app, db, minimal_record, recid_pid):
    ...
    stats = {
        ...
        "unique_views": 27,
        "version_unique_downloads": 280,
        "unique_download": 27,
    }

    with patch(
        "zenodo.modules.records.serializers.schemas.json" ".get_record_stats",
        return_value=stats,
    ) as m:
        obj = json_v1.transform_record(recid_pid, Record.create(minimal_record))
        assert m.called
