...
def test_is_embargoed(app):
    ...
    assert render_template_string("{{ dt is embargoed }}", dt=today) == "False"
    assert (
        render_template_string("{{ dt is embargoed }}", dt=today + timedelta(days=1))
        == "True"
    )
    assert (
        render_template_string(
            "{{ dt is embargoed(accessright='open') }}", dt=today + timedelta(days=1)
        )
        == "False"
    )
    assert (
    ...
    assert (
        render_template_string(
            "{{ dt is embargoed(accessright='embargoed') }}", dt=None
        )
        == "False"
    )
...
def test_accessright_category(app):
    ...
    assert render_template_string("{{ 'open'|accessright_category }}") == "success"


def test_accessright_title(app):
    """Test template filter."""
    assert render_template_string("{{ 'open'|accessright_title }}") == "Open Access"
...
def test_objecttype(app):
    ...
    assert (
        render_template_string(
            ...
            upload_type=dict(type="publication", subtype="book"),
        )
        == "Book"
    )
    ...
    assert (
        render_template_string(
            r"{% set t = upload_type|objecttype %}{{ t.title.en }}",
            upload_type=dict(type="publication"),
        )
        == "Publication"
    )
    ...
    assert (
        render_template_string(
            r"{% set t = upload_type|objecttype %}{{ t }}", upload_type=""
        )
        == "None"
    )
...
def test_local_doi(app):
    ...
    assert render_template_string("{{ '10.123/foo' is local_doi }}") == "True"
    assert render_template_string("{{ '10.1234/foo' is local_doi }}") == "False"
    assert render_template_string("{{ '10.5281/foo' is local_doi }}") == "True"
...
def test_relation_title(app):
    """Test relation title."""
    assert render_template_string("{{ 'isCitedBy'|relation_title }}") == "Cited by"
    assert (
        render_template_string("{{ 'nonExistingRelation'|relation_title }}")
        == "nonExistingRelation"
    )
...
def test_pid_url(app, sample_identifiers):
    ...
    for scheme, (_id, url) in sample_identifiers.items():
        assert (
            render_template_string(
                "{{{{ '{_id}'|pid_url(scheme='{scheme}') }}}}".format(
                    _id=_id, scheme=scheme
                )
            )
            == url
        )
    ...
    assert (
        render_template_string("{{ '10.123/foo'|pid_url }}")
        == "https://doi.org/10.123/foo"
    )
    assert (
        render_template_string("{{ 'doi: 10.123/foo'|pid_url(scheme='doi') }}")
        == "https://doi.org/10.123/foo"
    )
    assert render_template_string("{{ 'asfasdf'|pid_url }}") == ""
    assert (
        render_template_string(
    ...
    assert (
        render_template_string("{{ 'arXiv:1512.01558'|pid_url(scheme='arxiv') }}")
...
def test_records_ui_export(app, db, full_record):
    ...
    with app.test_client() as client:
        for f, val in formats.items():
            res = client.get(
                url_for("invenio_records_ui.recid_export", pid_value="12345", format=f)
            )
...
def test_citation_formatter_citeproc_get(api, api_client, es, db, full_record, users):
        ...
        (
            {
                ...
                "downloads": 28,
                "version_volume": 213000,
                "volume": 2000,
                "version_unique_views": 280,
                "unique_views": 27,
                "version_unique_downloads": 180,
                "unique_downloads": 25,
            },
            {
                "version_views": "31,200",
                "views": "2,130",
                "version_downloads": "54",
                "downloads": "28",
                "version_volume": "213.0 kB",
                "volume": "2.0 kB",
                "version_unique_views": "280",
                "unique_views": "27",
                "version_unique_downloads": "180",
...
def test_stats(app, db, minimal_record, stats, expected_result):
    ...
    with patch(
        "zenodo.modules.records.views.get_record_stats", return_value=stats
    ) as m:
        # local DOI
        record["doi"] = "10.5072/foo"
        template = render_template(
            "zenodo_records/box/record_stats.html", record=record
        )
