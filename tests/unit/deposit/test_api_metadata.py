...
def test_invalid_create(api_client, es, json_auth_headers, deposit_url, get_json):
    """Test invalid deposit creation."""
    client = api_client
    headers = json_auth_headers

    # Invalid deposits.
    cases = [dict(unknownkey="data", metadata={}), dict(metadat={})]
...
def test_unicode(
    ...
    test_data = dict(
        metadata=dict(
            ...
            contributors=[
                dict(name="Doe Sr.,  ن یک تست", affiliation="Atlantis", type="Other"),
                dict(
                    name="SmЭтith Sr., Marco",
                    affiliation="Atlantis",
                    type="DataCurator",
                ),
            ],
...
def test_validation(
    ...
    data = get_json(
        client.post(deposit_url, data=json.dumps(test_data), headers=headers), code=400
    )
