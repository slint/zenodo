...
def test_simple_rest_flow(
    mocker, api, api_client, db, es, locations, users, write_token, license_record
):
    ...
    response = client.delete(links["self"], headers=auth)
    assert response.status_code == 403
    ...
    files_list.reverse()
    response = client.put(links["files"], data=json.dumps(files_list), headers=auth)
    assert response.status_code == 403
...
def test_simple_delete(
    api_client, db, es, locations, json_auth_headers, auth_headers, deposit_url
):
    """Deletion."""
    client = api_client
    ...
    # Create
    links = get_json(
        client.post(deposit_url, data=json.dumps({}), headers=headers), code=201
    )["links"]
    current_search.flush_and_refresh(index="deposits")
    # Check list
    assert 1 == len(get_json(client.get(deposit_url, headers=headers), code=200))
    # Delete
    assert client.delete(links["self"], headers=auth_headers).status_code == 204
    current_search.flush_and_refresh(index="deposits")
    # Check list
    assert 0 == len(get_json(client.get(deposit_url, headers=headers), code=200))
    # Delete again
    assert client.delete(links["self"], headers=auth_headers).status_code == 410
...
def test_read_deposit_users(
    api, api_client, db, users, deposit, json_headers, user_info, status
):
...
def test_read_deposits_users(
    api, api_client, db, users, deposit, json_headers, user_info, status, count_deposit
):
    """Test read deposit by users."""
    with api.test_request_context():
        with api.test_client() as client:
            ...
            res = client.get(
                url_for("invenio_deposit_rest.depid_list"), headers=json_headers
            )
            assert res.status_code == status
            if user_info:
...
def test_update_deposits_users(
    api, api_client, db, users, deposit, json_headers, user_info, status
):
    """Test read deposit by users."""
    deposit_id = deposit["_deposit"]["id"]
...
def test_delete_deposits_users(
    api, api_client, db, es, users, deposit, json_headers, user_info, status
):
    """Test read deposit by users."""
    deposit_id = deposit["_deposit"]["id"]
    with api.test_request_context():
...
def test_versioning_rest_flow(
    ...
    response = client.post(
        links["files"],
        data={"file": make_file_fixture("test-1.txt"), "name": "test-1.txt"},
        headers=auth,
    )
    ...
    response = client.post(
        links["files"],
        data={"file": make_file_fixture("test-2.txt"), "name": "test-2.txt"},
        headers=auth,
    )
    assert response.status_code == 201
