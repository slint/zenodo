...
def test_bucket_create_delete(
    ...
    # Create deposit
    res = client.post(deposit_url, data=json.dumps(minimal_deposit), headers=headers)
    ...
    res = client.put(object_url, input_stream=BytesIO(b"viafilesrest"), headers=auth)
    assert res.status_code == 200
...
def test_bucket_create_publish(
    api_client,
    deposit,
    json_auth_headers,
    deposit_url,
    ...
    # Upload file
    res = client.put(
        links["bucket"] + "/test.txt", input_stream=BytesIO(b"testfile"), headers=auth
    )
    assert res.status_code == 200
...
def test_bucket_new_version(
    ...
    res = client.put(
        dep_v2_bucket + "/newfile.txt", input_stream=BytesIO(b"testfile2"), headers=auth
    )
    assert res.status_code == 200
    ...
    # Assert that all the buckets are different
    assert len(set([rec_v1_bucket, rec_v2_bucket, dep_v1_bucket, dep_v2_bucket])) == 4
