...
def test_double_minting_depid_recid(db):
    ...
    assert PersistentIdentifier.get("recid", pid.pid_value).status == PIDStatus.RESERVED
    db.session.commit()
...
@pytest.mark.parametrize("doi", ["1234/foo", "a"])
...
def test_unpublished_deposit_and_pid_deletion(deposit):
    ...
    pytest.raises(
        PIDDoesNotExistError, PersistentIdentifier.get, "recid", str(deposit["recid"])
    )
    depid = PersistentIdentifier.get("depid", str(deposit["_deposit"]["id"]))
...
def test_published_external_doi(db, deposit, deposit_file):
    ...
    recid = PersistentIdentifier.get("recid", str(deposit["recid"]))
    assert recid and recid.status == PIDStatus.REGISTERED and recid.has_object()
    # 2) a reserved external doi with object
    doi = PersistentIdentifier.get("doi", ext_doi1)
    assert doi and doi.status == PIDStatus.RESERVED and doi.has_object()
    ...
    pytest.raises(PIDDoesNotExistError, PersistentIdentifier.get, "doi", ext_doi1)
