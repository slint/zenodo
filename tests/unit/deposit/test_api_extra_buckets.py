...
def use_extra_formats_functions(
    ...
    if depid:
        ...
        response = api_client.options(
            extra_formats_urls["deposit"].format(depid), headers=extra_auth_headers
        )
        data = get_json(response, code=200)
        ...
        response = api_client.put(
            ...
            headers=extra_formats_headers["invalid-format"] + extra_auth_headers,
        )
...
def test_extra_formats_buckets(
    ...
    # Upload 1 files
    response = client.put(
        links["bucket"] + "/test1.txt", data="ctx", headers=extra_auth_headers
    )
    ...
    use_extra_formats_functions(extra_auth_headers, api_client, get_json, depid=depid)

    buckets = Bucket.query.all()
    assert len(buckets) == 2

    deposit = deposit_resolver.resolve(depid)[1]
    assert deposit["_buckets"]["extra_formats"] == str(deposit.extra_formats.bucket.id)
    # Publish deposition
    ...
    use_extra_formats_functions(
        extra_auth_headers, api_client, get_json, depid=depid, recid=first_version_recid
    )

    # Get newversion url
    data = get_json(client.get(links["self"], headers=extra_auth_headers), code=200)
    new_version_url = data["links"]["newversion"]

    # New Version
    data = get_json(client.post(new_version_url, headers=extra_auth_headers), code=201)
    links = data["links"]
    ...
    # Get latest version
    data = get_json(
        client.get(links["latest_draft"], headers=extra_auth_headers), code=200
    )
    links = data["links"]
    depid = data["record_id"]
    ...
    get_json(
        client.put(
            links["bucket"] + "/newfile.txt", data="newfile", headers=extra_auth_headers
        ),
        code=200,
    )
    ...
    assert {f["key"] for f in data} == {"application/foo+xml", "application/bar+xml"}
    ...
    assert first_record.extra_formats.bucket.id != new_record.extra_formats.bucket.id
