...
def test_edit_flow(
    ...
    archive_task_mock = mocker.patch("zenodo.modules.deposit.receivers.archive_sip")
    ...
    # Create deposit
    response = client.post(deposit_url, data=json.dumps(test_data), headers=headers)
    ...
    resdata = get_json(
        client.put(links["self"], data=json.dumps(newdata), headers=headers), code=200
    )
    ...
    doi_pid = PersistentIdentifier.get(pid_type="doi", pid_value="10.5072/zenodo.1")
    ...
    assert client.delete(links["self"], headers=auth_headers).status_code == 403
    ...
    resdata = get_json(
        client.put(links["self"], data=json.dumps(data), headers=headers), code=200
    )
    ...
    data = get_json(
        client.put(links["self"], data=json.dumps(data), headers=headers), code=400
    )
    ...
    assert get_json(
        ...
        code=200,
    )["metadata"]["communities"] == [{"identifier": "c1"}, {"identifier": "c3"}]
    ...
    assert postedit_data["metadata"]["communities"] == [{"identifier": "c1"}]

    # Was the second SIP sent for archiving?
    sip2 = (
        RecordSIP.query.filter_by(pid_id=recid_pid.id)
    ...
    assert (
        new_version_url == "http://localhost/deposit/depositions/2/actions/newversion"
    )
    ...
    # Discard
    data = get_json(client.post(links["discard"], headers=auth_headers), code=201)
    ...
    # New Version
    data = get_json(client.post(new_version_url, headers=auth_headers), code=201)
    links = data["links"]

    # Check if UI new version link is correct
    assert links["latest_draft_html"] == "http://localhost/deposit/3"

    # Get latest version
    data = get_json(client.get(links["latest_draft"], headers=auth_headers), code=200)
    links = data["links"]
    ...
    # Get the new record
    data = get_json(client.get(links["record"], headers=auth_headers), code=200)
...
def create_deposit(client, headers, auth_headers, deposit_url, get_json, data):
...
def test_edit_doi(
    ...
    record_url = url_for("invenio_records_rest.recid_item", pid_value=data["record_id"])

    # Create a persistent identifier
    PersistentIdentifier.create("doi", "10.1234/exists", status=PIDStatus.REGISTERED)
    db.session.commit()
    ...
    doi_exists = PersistentIdentifier.get(pid_type="doi", pid_value="10.1234/exists")
    doi_external = PersistentIdentifier.get(pid_type="doi", pid_value="10.1234/bar")
    ...
    doi_external = PersistentIdentifier.get(pid_type="doi", pid_value="10.4321/foo")
    assert doi_exists.status == PIDStatus.REGISTERED
    assert doi_external.status == PIDStatus.RESERVED
