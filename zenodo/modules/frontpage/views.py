...
blueprint = Blueprint(
    "zenodo_frontpage", __name__, url_prefix="", template_folder="templates"
)
...
def init_menu():
    ...
    item.register("invenio_deposit_ui.index", _("Upload"), order=2)
    item = current_menu.submenu("main.communities")
    item.register("invenio_communities.index", _("Communities"), order=3)
...
def index():
    ...
    if msg:
        flash(
            msg, category=current_app.config.get("FRONTPAGE_MESSAGE_CATEGORY", "info")
        )
