...
def update_oaisets_cache(cache, data):
    ...
    for spec in specs:
        if spec not in cache:
            ...
            if q.count():
                cache[spec] = OAISetCache(search_pattern=q.one().search_pattern)
...
def get_synced_sets(record, cache=None):
    ...
    other_sets = [
        ...
        for s in sets
        if not (
            s.startswith("user-") and not resolve_oaiset(s, cache=cache).search_pattern
        )
    ]
...
def comm_sets_match(record, cache=None):
    ...
    c_sets = [
        s
        for s in sets
        if s.startswith("user-") and not resolve_oaiset(s, cache=cache).search_pattern
    ]
...
def requires_sync(record, cache=None):
    ...
    return (
        (not oai.get("id"))
        or (oai.get("updated") is None)
        or (not comm_sets_match(record, cache=cache))
    )
...
def sync_record_oai(uuid, cache=None):
    ...
    oai_pid_q = PersistentIdentifier.query.filter_by(pid_type="oai", object_uuid=rec.id)
    if oai_pid_q.count() == 0:
        ...
        logger.info(
            "Minted new OAI PID ({pid}) for record {id}".format(pid=pid, id=uuid)
        )
        ...
        if not any(pid.pid_value.startswith(p) for p in managed_prefixes):
            ...
            logger.exception(
                ...
                "pointing to a different object ({id2})".format(
                    pid=pid, id=uuid, id2=str(pid.get_assigned_object()), recid=recid_s
                )
            )
            ...
            RecordIndexer().bulk_index([str(rec.id)])
            logger.info(
                "Matching OAI PID ({pid}) for record {id}".format(pid=pid, id=uuid)
            )
...
def recent_non_corrupted_revision(record):
    ...
    return next(
        filter(
            lambda rev: not has_corrupted_files_meta(rev), reversed(record.revisions)
        )
    )
...
def remove_oaiset_spec(record_uuid, spec):
    ...
    rec["_oai"]["sets"] = sorted([s for s in rec["_oai"].get("sets", []) if s != spec])
    rec["_oai"]["updated"] = datetime_to_datestamp(datetime.utcnow())
    if not rec["_oai"]["sets"]:
        del rec["_oai"]["sets"]
    rec.commit()
...
def iter_record_oai_tasks(query, spec, func):
    ...
    :type func: function
    """
    search = OAIServerSearch(index=current_app.config["OAISERVER_RECORD_INDEX"]).query(
        query
    )
...
def make_oai_task_group(oais):
    ...
    return group(
        ichain(
            iter_record_oai_tasks(spec_remove_q, oais.spec, remove_oaiset_spec),
...
def file_integrity_report():
    ...
    unhealthy_files = FileInstance.query.filter(
        sa.or_(FileInstance.last_check.is_(None), FileInstance.last_check.is_(False))
    ...
    if report:
        ...
        subject = "Zenodo files integrity report [{}]".format(datetime.now())
