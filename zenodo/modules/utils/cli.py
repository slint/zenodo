...
from .tasks import (
    has_corrupted_files_meta,
    repair_record_metadata,
    sync_record_oai,
    update_oaisets_cache,
...
def add_file(recid, fp, replace_existing):
    ...
    click.echo(
        click.style(
            ...
            u"  RECID: {recid}\n"
            u"  UUID: {uuid}\n"
            u"".format(recid=record["recid"], title=record["title"], uuid=record.id),
            fg="green",
        )
    )
    if replace_existing and obj is not None:
        ...
        click.echo(
            click.style(
                ...
                u"  bucket: {bucket}\n"
                u"  size: {size}\n"
                u"".format(key=obj.key, bucket=obj.bucket, size=obj.file.size),
                fg="green",
            )
        )
...
def remove_file(recid, key=None, index=None):
    ...
    if obj is None:
        click.echo(
            click.style(
                u'File with key "{key}" not found.'.format(key=key, recid=recid),
                fg="red",
            )
        )
        return
    ...
    click.echo(
        click.style(
            ...
            u"  {checksum}\n"
            u"  bucket: {bucket}\n"
            u"".format(key=key, checksum=obj.file.checksum, bucket=bucket.id),
            fg="green",
        )
    )
...
def rename_file(recid, key, new_key):
    ...
    if obj is None:
        click.echo(
            click.style(u'File with key "{key}" not found.'.format(key=key), fg="red")
        )
        return
    ...
    if new_obj is not None:
        click.echo(
            click.style(
                u'File with key "{key}" already exists.'.format(key=new_key), fg="red"
            )
        )
        return

    if click.confirm(
        u'Rename "{key}" to "{new_key}" on bucket {bucket}.'
        u" Continue?".format(key=obj.key, new_key=new_key, bucket=bucket.id)
    ):
...
@with_appcontext
def attach_file(file_id, pid_type1, pid_value1, key1, pid_type2, pid_value2, key2):
    ...
    """
    assert (file_id or (pid_type1 and pid_value1 and key1)) and (
        pid_type2 and pid_value2 and key2
    )
    ...
    if not file_id:
        resolver = record_resolver if pid_type1 == "recid" else deposit_resolver
        ...
        if obj1 is None:
            click.echo(
                click.style(
                    u'File with key "{key}" not found.'.format(key=key1), fg="red"
                )
            )
            return
    ...
    if obj2 is not None:
        click.echo(
            click.style(
                u'File with key "{key}" already exists on'
                u" bucket {bucket}.".format(key=key2, bucket=bucket2.id),
                fg="red",
            )
        )
        return
    ...
    if click.confirm(
        u'Attaching file "{file_id}" to bucket {bucket2}'
        u' as "{key2}". Continue?'.format(
            file_id=file_id, key2=key2, bucket2=bucket2.id
        )
    ):
...
def list_files(recid):
    ...
    pid, record = record_resolver.resolve(recid)
    click.echo(
        u"Files for record {recid} (UUID:{uuid}) ({cnt} file(s)):\n"
...
def repair_corrupted_metadata(eager, uuid):
    ...
    if uuid:
        ...
        if not click.confirm(
            "Will update {cnt} records. Continue?".format(cnt=len(uuids))
        ):
            return
...
@with_appcontext
def split_openaire_grants_dump(
    source, target_prefix, grants_per_file=None, sqlite_write_rows_buffer=None
):
    ...
    split_files = grants_dump.split(target_prefix, grants_per_file=grants_per_file)
    total_rows = 0
    for filepath, row_count in split_files:
        total_rows += row_count
        click.secho(
            "{0} - {1} (Total: {2})".format(filepath, row_count, total_rows), fg="blue"
        )
...
def update_openaire_communities(path):
    ...
    mapping, unresolved_communities = mapping_updater.update_communities_mapping()
    click.secho(
        "Communities not found:\n{0}".format(
            json.dumps(unresolved_communities, indent=4, separators=(", ", ": "))
        )
    )
    click.secho(
        "{0}".format(json.dumps(mapping, indent=4, separators=(", ", ": "))), fg="blue"
    )
