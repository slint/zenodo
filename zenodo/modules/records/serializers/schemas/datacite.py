...
class PersonSchema(Schema):
    ...
    def get_nameidentifier(self, obj):
        ...
        if obj.get("gnd"):
            return {"nameIdentifier": obj.get("gnd"), "nameIdentifierScheme": "GND"}
        return {}
...
class DataCiteSchema(Schema):
    ...
    def get_identifier(self, obj):
        ...
        if is_doi_locally_managed(doi):
            return {"identifier": doi, "identifierType": "DOI"}
        else:
    ...
    def get_descriptions(self, obj):
        ...
        max_descr_size = current_app.config.get("DATACITE_MAX_DESCRIPTION_SIZE", 20000)
        if desc:
            items.append(
                {"description": desc[:max_descr_size], "descriptionType": "Abstract"}
            )
        notes = obj["metadata"].get("notes")
        if notes:
            items.append(
                {"description": notes[:max_descr_size], "descriptionType": "Other"}
            )
            ...
            items.append(
                {
                    "description": json.dumps(
                        {
                            "references": [
                                r["raw_reference"] for r in refs if "raw_reference" in r
                            ]
                        }
                    )[:max_descr_size],
        ...
        if method:
            items.append(
                {"description": method[:max_descr_size], "descriptionType": "Methods"}
            )
        return items
    ...
    def get_rights(self, obj):
        ...
        if license_url and license_text:
            items.append({"rightsURI": license_url, "rights": license_text})
        ...
        items.append(
            {
                ...
                "rights": "{0} access".format(obj["metadata"]["access_right"]).title(),
            }
        )
        return items
    ...
    def get_related_identifiers(self, obj):
        ...
        if not is_doi_locally_managed(doi):
            items.append(
                s.dump(
                    {"identifier": doi, "scheme": "doi", "relation": "IsIdenticalTo"}
                ).data
            )
    ...
    def get_subjects(self, obj):
        ...
        for s in obj["metadata"].get("subjects", []):
            items.append({"subject": s["identifier"], "subjectScheme": s["scheme"]})
    ...
    def get_dates(self, obj):
        ...
        schema = schema_cls()
        dates = []
        if obj["metadata"]["access_right"] == "embargoed" and obj["metadata"].get(
            "embargo_date"
        ):
            dates.append(
                schema.dump(
                    dict(date=obj["metadata"]["embargo_date"], type="Available")
                ).data
            )

            dates.append(
                schema.dump(
                    dict(date=obj["metadata"]["publication_date"], type="Accepted")
                ).data
            )
        else:
            dates.append(
                schema.dump(
                    dict(date=obj["metadata"]["publication_date"], type="Issued")
                ).data
            )
                    ...
                    dates.append(
                        schema.dump(
                            dict(
                                date=start,
                                type=date_type,
                                info=interval.get("description", missing),
                            )
                        ).data
                    )
                else:
...
class DataCiteSchemaV1(DataCiteSchema):
    ...
    creators = fields.List(fields.Nested(CreatorSchema), attribute="metadata.creators")
    ...
    def get_contributors(self, obj):
        ...
        contributors.extend(
            [inject_type(c) for c in obj["metadata"].get("thesis_supervisors", [])]
        )
    ...
    def get_locations(self, obj):
        ...
        for l in obj["metadata"].get("locations", []):
            ...
            if l.get("lat") and l.get("lon"):
                location["geoLocationPoint"] = "{} {}".format(l["lat"], l["lon"])
...
class PersonSchemav4(Schema):
    ...
    affiliations = fields.List(fields.Str(), attribute="affiliation")
    ...
    def get_nameidentifiers(self, obj):
        ...
        if obj.get("gnd"):
            name_identifiers.append(
                {"nameIdentifier": obj.get("gnd"), "nameIdentifierScheme": "GND"}
            )
...
class DataCiteSchemaV4(DataCiteSchema):
    ...
    creators = fields.List(
        fields.Nested(CreatorSchemav4), attribute="metadata.creators"
    )
    ...
    def get_contributors(self, obj):
        ...
        contributors.extend(
            [
                inject_type(c)
                for c in obj["metadata"].get("thesis").get("supervisors", [])
            ]
        )
