...
class RecordSchemaMARC21(Schema):
    ...
    terms_governing_use_and_reproduction_note = fields.Function(
        lambda o: dict(
            uniform_resource_identifier=o["metadata"].get("license", {}).get("url"),
            terms_governing_use_and_reproduction=o["metadata"]
            .get("license", {})
            .get("title"),
        )
    )

    title_statement = fields.Function(lambda o: dict(title=o["metadata"].get("title")))
    ...
    summary = fields.Function(lambda o: dict(summary=o["metadata"].get("description")))

    host_item_entry = fields.Method("get_host_item_entry")

    dissertation_note = fields.Function(
        lambda o: dict(
            name_of_granting_institution=o["metadata"]
            .get("thesis", {})
            .get("university")
        )
    )
    ...
    language_code = fields.Function(
        lambda o: dict(
            language_code_of_text_sound_track_or_separate_title=o["metadata"].get(
                "language"
            )
        )
    )
    ...
    def get_subject_added_entry_topical_term(self, o):
        ...
        for s in o["metadata"].get("subjects", []):
            res.append(_subject(s.get("term"), s.get("identifier"), s.get("scheme")))
    ...
    def get_other_standard_identifier(self, o):
        ...
        for id_ in m.get("alternate_identifiers", []):
            res.append(
                stdid(id_.get("identifier"), id_.get("scheme"), q="alternateidentifier")
            )
    ...
    def _get_personal_name(self, v, relator_code=None):
        ...
        return dict(
            ...
            authority_record_control_number_or_standard_number=[
                "({0}){1}".format(scheme, identifier) for (scheme, identifier) in ids
            ],
