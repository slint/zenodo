...
class PersonSchemaV1(Schema, StrictKeysMixin):
    ...
    def validate_data(self, data):
        ...
        name = data.get("name")
        if not name:
            raise ValidationError(_("Name is required."), field_names=["name"])
...
class ContributorSchemaV1(PersonSchemaV1):
    ...
    def validate_type(self, value):
        """Validate the type."""
        if value not in current_app.config["DEPOSIT_CONTRIBUTOR_DATACITE2MARC"]:
            raise ValidationError(_("Invalid contributor type."), field_names=["type"])
...
class IdentifierSchemaV1(Schema, StrictKeysMixin):
    ...
    def normalize_identifier(self, data):
        ...
        data["identifier"] = idutils.normalize_pid(data["identifier"], data["scheme"])
    ...
    def validate_data(self, data):
        ...
        scheme = data.get("scheme")
        if not id_:
            raise ValidationError("Identifier is required.", field_names=["identifier"])

        schemes = idutils.detect_identifier_schemes(id_)
        if not schemes:
            raise ValidationError(
                "Not a valid persistent identifier.", field_names=["identifier"]
            )
        if scheme not in schemes:
            raise ValidationError(
                "Not a valid {0} identifier.".format(scheme), field_names=["identifier"]
            )
...
class RelatedIdentifierSchemaV1(IdentifierSchemaV1):
    ...
    relation = fields.Str(
        required=True,
        validate=validate.OneOf(choices=[x[0] for x in ZENODO_RELATION_TYPES]),
    )
...
class CommonMetadataSchemaV1(Schema, StrictKeysMixin, RefResolverMixin):
    ...
    creators = fields.Nested(PersonSchemaV1, many=True, validate=validate.Length(min=1))
    dates = fields.List(fields.Nested(DateSchemaV1), validate=validate.Length(min=1))
    ...
    alternate_identifiers = fields.Nested(AlternateIdentifierSchemaV1, many=True)
    ...
    def validate_dates(self, value):
        ...
        for interval in value:
            start = (
                arrow.get(interval.get("start"), "YYYY-MM-DD").date()
                if interval.get("start")
                else None
            )
            end = (
                arrow.get(interval.get("end"), "YYYY-MM-DD").date()
                if interval.get("end")
                else None
            )
    ...
    def validate_license_ref(self, value):
        ...
        if not self.validate_jsonref(value):
            raise ValidationError(_("Invalid choice."), field_names=["license"])
    ...
    def validate_grants_ref(self, values):
        ...
        for v in values:
            if not self.validate_jsonref(v):
                raise ValidationError(_("Invalid grant."), field_names=["grants"])
    ...
    def validate_doi(self, value):
        ...
        if value and has_request_context():
            ...
            err = ValidationError(
                _("DOI already exists in Zenodo."), field_names=["doi"]
            )
            ...
            try:
                recid_pid = PersistentIdentifier.get("recid", self.context["recid"])
            except PIDDoesNotExistError:
    ...
    def validate_license(self, data):
        ...
        if acc in [AccessRight.OPEN, AccessRight.EMBARGOED] and "license" not in data:
            raise ValidationError(
    ...
    def load_custom(self, obj):
        ...
        for term, values in obj.items():
            if term not in valid_vocabulary:
                raise ValidationError(
                    'Zenodo does not support "{0}" as a custom metadata term.'.format(
                        term
                    ),
                    field_names=["custom"],
                )
            ...
            if not isinstance(values, list):
                raise ValidationError(
                    'Term "{0}" should be of type array.'.format(term),
                    field_names=["custom"],
                )
            if len(values) == 0:
                raise ValidationError(
                    'No values were provided for term "{0}".'.format(term),
                    field_names=["custom"],
                )
            for value in values:
                if not isinstance(value, term_type):
                    raise ValidationError(
                        'Invalid type for term "{0}", should be "{1}".'.format(
                            term, valid_terms[term]["term_type"]
                        ),
                        field_names=["custom"],
                    )
            ...
            if len(values) > 1 and not multiple_values:
                raise ValidationError(
                    'The term "{0}" accepts only one value.'.format(term),
                    field_names=["custom"],
                )
        return obj
    ...
    def preload_accessrights(self, data):
        ...
        if data.get("access_right") not in [AccessRight.OPEN, AccessRight.EMBARGOED]:
    ...
    def postload_keywords_filter(self, data):
        ...
        if "keywords" in data:
            data["keywords"] = [kw for kw in data["keywords"] if kw.strip()]
    ...
    def postload_references(self, data):
        ...
        if "references" in data:
            data["references"] = [
                {"raw_reference": ref} for ref in data["references"] if ref.strip()
            ]
    ...
    def _dump_relation_links(self, metadata):
        ...
        if relations:
            ...
            if version_info:
                ...
                if last_child:
                    links["latest"] = api_link_for("record", id=last_child["pid_value"])
