...
class LegacyMetadataSchemaV1(common.CommonMetadataSchemaV1):
    ...
    publication_type = fields.Method(
        ...
        validate=validate.OneOf(choices=ObjectType.get_subtypes("publication")),
    )
    ...
    openaire_type = fields.Method(
        "dump_openaire_type", attribute="resource_type.openaire_subtype"
    )
    ...
    conference_session_part = SanitizedUnicode(attribute="meeting.session_part")
    ...
    def dump_grants(self, obj):
        ...
        for g in obj.get("grants", []):
            if (
                g.get("program", {}) == "FP7"
                and g.get("funder", {}).get("doi") == "10.13039/501100000780"
            ):
    ...
    def load_grants(self, data):
        ...
        return [
            ...
            for grant_id in result
        ] or missing

    def dump_communities(self, obj):
        """Dump communities type."""
        return [dict(identifier=x) for x in obj.get("communities", [])] or missing
    ...
    def load_communities(self, data):
        ...
        comm_ids = list(sorted([x["identifier"] for x in data if x.get("identifier")]))
    ...
    def dump_prereservedoi(self, obj):
        ...
        if recid:
            ...
            return dict(recid=recid, doi=doi_generator(recid, prefix=prefix))
        return missing
    ...
    def validate_communities(self, values):
        ...
        for v in values:
            if not isinstance(v, six.string_types):
                raise ValidationError(
                    _("Invalid community identifier."), field_names=["communities"]
                )
    ...
    def validate_data(self, obj):
        ...
        if ObjectType.get_by_dict(type_dict) is None:
            raise ValidationError(
                _("Invalid upload, publication or image type."), field_names=field_names
            )
        if not is_valid_openaire_type(
            obj.get("resource_type", {}), obj.get("communities", [])
        ):
            raise ValidationError(
                _("Invalid OpenAIRE subtype."), field_names=["openaire_subtype"]
            )
...
class LegacyRecordSchemaV1(common.CommonRecordSchemaV1):
    ...
    submitted = fields.Function(
        lambda o: o["metadata"].get("_deposit", {}).get("pid") is not None,
        dump_only=True,
    )
    title = SanitizedUnicode(attribute="metadata.title", default="", dump_only=True)
