...
class Bibtex(object):
    ...
    def _format_publication(self):
        formats = {
            "book": [self._format_book, self._format_booklet, self._format_misc],
            "section": [self._format_misc],
            "conferencepaper": [
                self._format_inproceedings,
                self._format_proceedings,
                self._format_misc,
            ],
            "article": [self._format_article, self._format_misc],
            "patent": self._format_misc,
            "preprint": [self._format_unpublished, self._format_misc],
            "report": [self._format_misc],
            "thesis": [self._format_thesis, self._format_misc],
            "technicalnote": [self._format_manual, self._format_misc],
            "workingpaper": [self._format_unpublished, self._format_misc],
    ...
    def _format_article(self):
        ...
        ign_fields = ["doi", "url"]
        return self._format_entry(name, req_fields, opt_fields, ign_fields)
    ...
    def _get_citation_key(self):
        ...
        if "recid" in self.record:
            ...
            if authors:
                ...
                name = first_author.get("familyname", first_author.get("name"))
                pubdate = self.record.get("publication_date", None)
                if pubdate:
                    year = "{}_{}".format(iso2dt(pubdate).year, self.record["recid"])
                else:
                    year = self.record["recid"]

                return "{0}_{1}".format(
                    slugify(name, separator="_", max_length=40), year
                )
            else:
    ...
    def _get_address(self):
        ...
        if "imprint" in self.record and "place" in self.record["imprint"]:
    ...
    def _get_booktitle(self):
        ...
        if "part_of" in self.record and "title" in self.record["part_of"]:
    ...
    def _get_journal(self):
        ...
        if "journal" in self.record and "title" in self.record["journal"]:
    ...
    def _get_number(self):
        ...
        if "journal" in self.record and "issue" in self.record["journal"]:
    ...
    def _get_pages(self):
        ...
        if "journal" in self.record and "pages" in self.record["journal"]:
            return self.record["journal"]["pages"]
        elif "part_of" in self.record and "pages" in self.record["part_of"]:
    ...
    def _get_publisher(self):
        ...
        if "imprint" in self.record and "publisher" in self.record["imprint"]:
            return self.record["imprint"]["publisher"]
        elif "part_of" in self.record and "publisher" in self.record["part_of"]:
    ...
    def _get_url(self):
        ...
        return "https://doi.org/%s" % self.record["doi"] if "doi" in self.record else ""

    def _get_venue(self):
        """Return conference's venue."""
        if "meeting" in self.record and "place" in self.record["meeting"]:
