...
class AccessRight(object):
    ...
    _description = {
        ...
        EMBARGOED: _(
            "Files are currently under embargo but will be publicly "
            "accessible after {date}."
        ),
        RESTRICTED: _(
            "You may request access to the files in this upload, "
    ...
    def get(cls, value, embargo_date=None):
        """Get access right."""
        if (
            embargo_date is not None
            and cls.EMBARGOED == value
            and not cls.is_embargoed(embargo_date)
        ):
            return cls.OPEN
    ...
    def as_description(cls, value, embargo_date=None):
        ...
        return cls._description[value].format(date=format_date(embargo_date, "long"))
    ...
    def get_expired_embargos(cls):
        ...
        s = (
            RecordsSearch(using=current_search_client, index=endpoint["search_index"])
            .query(
                "query_string",
...
class ObjectType(object):
    ...
    def _load_data(cls):
        ...
        if cls.index_id is None:
            with open(join(dirname(__file__), "data", "objecttypes.json")) as fp:
                data = json.load(fp)
    ...
    def get(cls, value):
        ...
        try:
            return JsonRef.replace_refs(
                cls.index_internal_id[value], jsonschema=True, loader=cls._jsonloader
            )
        except KeyError:
            return None
    ...
    def get_by_dict(cls, value):
        ...
        if "subtype" in value:
            ...
            internal_id = "{0}-{1}".format(
                value.get("type", ""), value.get("subtype", "")
            )
        else:
