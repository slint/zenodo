...
def zenodo_record_minter(record_uuid, data):
    ...
    if "recid" in data:
        ...
        recid = RecordIdProvider.create(object_type="rec", object_uuid=record_uuid).pid
...
def zenodo_concept_doi_minter(record_uuid, data):
    ...
    if is_local_doi(doi):
        ...
        conceptdoi_pid = PersistentIdentifier.query.filter_by(
            pid_type="doi", pid_value=conceptdoi
        ).one_or_none()
        ...
        if not conceptdoi_pid:
            ...
            conceptdoi_pid.assign(
                object_type="rec", object_uuid=record_uuid, overwrite=True
            )
