...
def openaire_direct_index_after_publish(sender, action=None, pid=None, deposit=None):
    """Send published record for direct indexing at OpenAIRE."""
    if action == "publish" and current_app.config["OPENAIRE_DIRECT_INDEXING_ENABLED"]:
        _, record = deposit.fetch_published()
        openaire_direct_index.delay(record_uuid=str(record.id))


def sipstore_write_files_after_publish(sender, action=None, pid=None, deposit=None):
    """Send the SIP for archiving."""
    if action == "publish" and current_app.config["SIPSTORE_ARCHIVER_WRITING_ENABLED"]:
        recid_pid, record = deposit.fetch_published()
        sip = (
