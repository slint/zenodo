...
blueprint = Blueprint(
    "zenodo_redirector", __name__, template_folder="templates", static_folder="static"
)
...
def configure_search_handler(blueprint_setup):
    ...
    blueprint_setup.app.view_functions["invenio_search_ui.search"] = search_handler
...
def collections_search_redirect():
    """."""
    values = request.args.to_dict()
    type, subtype = ZENODO_TYPE_SUBTYPE_LEGACY.get(values.pop("cc"), ("", None))
    values["type"] = type
    if subtype:
        values["subtype"] = subtype
    _pop_p(values)
