...
blueprint = Blueprint("zenodo_support", __name__, template_folder="templates")
...
def support():
    ...
    if form.validate_on_submit():
        ...
        if attachments and not check_attachment_size(attachments):
            form.attachments.errors.append(
                "File size exceeded. "
                "Please add URLs to the files "
                "or make a smaller selection."
            )
        else:
            context = {"user_id": current_user.get_id(), "info": form.data, "uap": uap}
