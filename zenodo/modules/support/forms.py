...
class ContactForm(FlaskForm):
    ...
    description = TextAreaField(
        _("How can we help?"), description=_("Required."), filters=[strip_filter]
    )

    include_os_browser = BooleanField(_("Browser & OS"), default="checked")

    submit = SubmitField(_("Send Request"))

    attachments = FileField(_("Attachments"), render_kw={"multiple": True})
...
def contact_form_factory():
    ...
    if current_user.is_authenticated:
        ...
        form.name.data = form.name.data or (
            current_user.profile.full_name if current_user.profile else None
        )
