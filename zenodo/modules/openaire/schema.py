...
class RecordSchemaOpenAIREJSON(Schema):
    ...
    def _openaire_type(self, obj):
        return ObjectType.get_by_dict(obj.get("metadata", {}).get("resource_type")).get(
            "openaire"
        )
    ...
    def get_original_id(self, obj):
        ...
        if oatype:
            return openaire_original_id(obj.get("metadata", {}), oatype["type"])[1]
        return missing
    ...
    def get_license_code(self, obj):
        ...
        return self.LICENSE_MAPPING.get(metadata.get("access_right"), "UNKNOWN")
